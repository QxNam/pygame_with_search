...
class Point():
    ...
    def __init__(self, x: int, y: int, l=None, par = None):
        self.x = x
        self.y = y
        self.l = l
        self.par = par

     # hàm xem thông tin tọa độ
    def getPoint(self):
...
class Search():
    ...
    def viewInfo(self, s: tuple, g: tuple):
        d = self.runAll(s, g)
        name = [d[i][0] for i in range(4)]
        time = [d[i][1] for i in range(4)]
        number_of_steps = [len(d[i][2]) for i in range(4)]
